@extends('layouts.app')
@section('title', __('ui.pg_edit_task'))
@section('page-title', __('ui.pg_edit_task'))
@section('content')
        ...
        <div class="row g-3">
            ...
            <div class="col-12">
                ...
                <select name="project_id" class="form-select" required>
                    @foreach($projects as $p)
                        <option value="{{ $p->id }}" {{ old('project_id', $task->project_id) == $p->id ? 'selected' : '' }}>{{ $p->name }} — {{ $p->company?->name }}</option>
                    @endforeach
                </select>
            </div>
            <div class="col-md-4">
            ...
            <div class="col-md-4">
                ...
                <select name="priority" class="form-select">
                    ...
                    <option value="{{ $pr }}" {{ old('priority', $task->priority) == $pr ? 'selected' : '' }}>{{ __('ui.priority_'.$pr) }}</option>
                    @endforeach
                </select>
            </div>
            <div class="col-md-4">
                ...
                <select name="status" class="form-select">
                    ...
                    <option value="{{ $s }}" {{ old('status', $task->status) == $s ? 'selected' : '' }}>{{ __('ui.status_'.$s) }}</option>
                    @endforeach
                </select>
            </div>
            <div class="col-md-6">
            ...
            <div class="col-md-6">
                ...
                <input type="date" name="completed_date" class="form-control" value="{{ old('completed_date', $task->completed_date?->format('Y-m-d')) }}">
            </div>
            <div class="col-12">
            ...
            <div class="col-12 d-flex gap-2">
                <button type="submit" class="btn btn-primary">{{ __('ui.btn_update_task') }}</button>
